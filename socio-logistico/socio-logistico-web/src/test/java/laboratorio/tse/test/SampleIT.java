...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "socio-logistico-ear.ear");

        // Current directory is the root of the "socio-logistico-web" project. Go up one level
        // and enter the "socio-logistico-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../socio-logistico-ejb/target/socio-logistico-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "socio-logistico-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/socio-logistico-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "socio-logistico-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("laboratorio.tse.test");
