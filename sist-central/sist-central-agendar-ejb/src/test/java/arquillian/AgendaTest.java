...
public class AgendaTest {
    ...
    @EJB
    private AgendaServiceRemote agendaService;
    ...
    public void should_confirm_reserva() {
    	List<VacunatorioTieneAgendaDTO> agendasVacunatorios = agendaService.findAgendasParaCiudadanoPorDepartamento("Corona Virus", 50, Trabajo.SALUD, Departamento.Montevideo);
    	...
    	assertEquals(5, agendaService.findByNombrePlan("Plan vacunacion covid 19").size());
    	
    	List<Intervalo> intervalos = null;
    	try {
        	intervalos = agendaService.getIntervalos(4, LocalDate.of(2021, 6, 21));
    	}
    	...
    	try {
        	intervalos = agendaService.getIntervalos(4, LocalDate.of(2021, 6, 22));
    	}
    	catch(RuntimeException e){
    		//todo ok
    	}
    	...
    	List<Reserva> reservas = reservaService.efectuarReserva(intervalo, 52050756);
