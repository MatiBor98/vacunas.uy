package laboratorio.tse.Vacunatorio.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "Vacunatorio-ear.ear");

        // Current directory is the root of the "Vacunatorio-web" project. Go up one level
        // and enter the "Vacunatorio-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../Vacunatorio-ejb/target/Vacunatorio-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "Vacunatorio-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/Vacunatorio-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "Vacunatorio-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("laboratorio.tse.Vacunatorio.test");
